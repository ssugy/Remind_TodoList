...
// 실제 분류 모델 클래스
class ClassifierHandNum(
    val context: Context
) {
    // 에셋폴더 파일이름 저장
    companion object{
        val MODEL_NAME_HAND = "keras_model_cnn.tflite"
    }
    ...
    // 손글씨 기능은 그냥 이대로 두고, 이미지 분석 기능만 서포트 라이브러리 사용 예정
    fun initHandNumber(){
        val model = loadModelFile(MODEL_NAME_HAND)     // 서포트 라이브러리 추가로 이제 사용하지 않음
